...
class SchedulingService {
     ...
     *
     * In short, go to kdn-auto-leech.php and make sure SchedulingService is initialized when your new CRON event
    ...
    public $eventCollectUrls   = 'kdn_event_collect_urls';
    public $eventCrawlPost     = 'kdn_event_crawl_post';
    public $eventRecrawlPost   = 'kdn_event_recrawl_post';
    public $eventDeletePosts   = 'kdn_event_delete_posts';
    ...
    public function executeEventCollectUrls($bypassInactiveScheduling = false) {
        ...
        $this->execute($urlSaver->optionLastCheckedSiteId,
            ...
            function() {
                return $this->getSetting('_kdn_run_count_url_collection', 1);
            },
    ...
    public function executeEventCrawlPost($bypassInactiveScheduling = false) {
        ...
        $this->execute($postSaver->optionLastCrawledSiteId, null,
            // Get required run count callback
            function() {
                return $this->getSetting('_kdn_run_count_post_crawl', 1);
            },
    ...
    public function executeEventRecrawlPost($bypassInactiveScheduling = false) {
        ...
        $this->execute($postSaver->optionLastRecrawledSiteId, null,
            // Get required run count callback
            function() {
                return $this->getSetting('_kdn_run_count_post_recrawl', 1);
            },
            ...
            function($siteIdToCheck) use (&$postSaver, &$results) {
                ...
                $maxRecrawlCount = (int) $this->getSetting('_kdn_max_recrawl_count', 0);
                ...
                $timeBetweenRecrawlsInMin = (int) $this->getSetting('_kdn_min_time_between_two_recrawls_in_min');
                ...
                $newerThanInMin = (int) $this->getSetting('_kdn_recrawl_posts_newer_than_in_min');
    ...
    public function executeEventDeletePosts($bypassInactiveScheduling = false) {
        ...
        $optionKeyMaxPostsToDelete = '_kdn_max_post_count_per_post_delete_event';
        ...
        $this->execute($this->optionKeyLastPostDeletedSiteId, null,
            ...
            function($siteIdToCheck) use (&$totalDeleteCount, &$remaining) {
                ...
                $optionKeyOlderThan = '_kdn_delete_posts_older_than_in_min';
                ...
                $isDeleteAttachments = $this->getSetting('_kdn_is_delete_post_attachments');
                ...
                foreach($postIds as $id) {
                    ...
                    /**
                     * Fires just before delete a post.
                     *
                     * @param int   $id                 The ID of post
                     * @param int   $siteIdToCheck      The ID of site (campaign)
                     *
                     * @since 2.2.3
                     */
                    do_action('kdn/schedule/before_delete', $id, $siteIdToCheck);

                    /**
                     * Filter bypass a delete post event. If true, the post will be not deleted.
                     *
                     * @param int   $id                 The ID of post
                     * @param int   $siteIdToCheck      The ID of site (campaign)
                     *
                     * @since 2.2.3
                     */
                    $byPass = apply_filters('kdn/schedule/bypass_delete', false, $id, $siteIdToCheck);

                    // When bypass delete, continue delete the next post
                    if ($byPass) {
                        Factory::databaseService()->setUrlBypassDelete($id);
                        continue;
                    }

                    /**
                     * Fires just after bypass and continue delete a post.
                     *
                     * @param int   $id                 The ID of post
                     * @param int   $siteIdToCheck      The ID of site (campaign)
                     *
                     * @since 2.2.3
                     */
                    do_action('kdn/schedule/continue_delete', $id, $siteIdToCheck);
                    ...
                    if($isDeleteAttachments) {
                        ...
                        foreach(get_attached_media('', $id) as $mediaPost) wp_delete_post($mediaPost->ID, true);
                    }
                    ...
                    /**
                     * Fires just after delete a post.
                     *
                     * @param int   $id                 The ID of post
                     * @param int   $siteIdToCheck      The ID of site (campaign)
                     *
                     * @since 2.2.3
                     */
                    do_action('kdn/schedule/after_delete', $id, $siteIdToCheck);
                }
...
//                        error_log("KDN Auto Leech - Nothing to do. Try another site. Option: {$lastCheckedSiteIdOptionName}, Current Site ID: {$siteIdToCheck} ");
                        $run = true;
                        $siteIdToCheck = null;
                        break;
    ...
    public function scheduleEvents() {
        $intervalCollectUrls = get_option('_kdn_interval_url_collection');
        $intervalCrawlPosts  = get_option('_kdn_interval_post_crawl');
    ...
    public function scheduleRecrawlingEvent() {
        $interval = get_option('_kdn_interval_post_recrawl');
    ...
    public function scheduleDeletingEvent() {
        $interval = get_option('_kdn_interval_post_delete');
    ...
    public function getIntervals() {
        $this->intervals = [
            ...
            '_kdn_1_minute'    =>  [_kdn('Every minute'),        60],
            '_kdn_2_minutes'   =>  [_kdn('Every 2 minutes'),     2 * 60],
            '_kdn_3_minutes'   =>  [_kdn('Every 3 minutes'),     3 * 60],
            '_kdn_5_minutes'   =>  [_kdn('Every 5 minutes'),     5 * 60],
            '_kdn_10_minutes'  =>  [_kdn('Every 10 minutes'),    10 * 60],
            '_kdn_15_minutes'  =>  [_kdn('Every 15 minutes'),    15 * 60],
            '_kdn_20_minutes'  =>  [_kdn('Every 20 minutes'),    20 * 60],
            '_kdn_30_minutes'  =>  [_kdn('Every 30 minutes'),    30 * 60],
            '_kdn_45_minutes'  =>  [_kdn('Every 45 minutes'),    45 * 60],
            '_kdn_1_hour'      =>  [_kdn('Every hour'),          60 * 60],
            '_kdn_2_hours'     =>  [_kdn('Every 2 hours'),       2 * 60 * 60],
            '_kdn_3_hours'     =>  [_kdn('Every 3 hours'),       3 * 60 * 60],
            '_kdn_4_hours'     =>  [_kdn('Every 4 hours'),       4 * 60 * 60],
            '_kdn_6_hours'     =>  [_kdn('Every 6 hours'),       6 * 60 * 60],
            '_kdn_12_hours'    =>  [_kdn('Twice a day'),         12 * 60 * 60],
            '_kdn_1_day'       =>  [_kdn('Once a day'),          24 * 60 * 60],
            '_kdn_2_days'      =>  [_kdn('Every 2 days'),        2 * 24 * 60 * 60],
            '_kdn_1_week'      =>  [_kdn('Once a week'),         7 * 24 * 60 * 60],
            '_kdn_2_weeks'     =>  [_kdn('Every 2 weeks'),       2 * 7 * 24 * 60 * 60],
            '_kdn_1_month'     =>  [_kdn('Once a month'),        4 * 7 * 24 * 60 * 60],
        ];
