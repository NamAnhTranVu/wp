...
use KDNAutoLeech\Migrations\DatabaseMigrator;
...
class DatabaseService {
    ...
    private $dbVersionOptionName = '_kdn_db_version';

    /** @var string Name for the URLs table */
    private $dbTableUrls = 'kdn_urls';
    ...
    public function addUrl($postId, $url, $thumbnailUrl, $categoryId, $isSaved = false) {
        ...
        // Get duplicate types
        $duplicateTypes = get_post_meta($postId, '_duplicate_check_types', true);
        $duplicateTypeUrl = isset($duplicateTypes[0]['url']);

        // If the URL is added and duplicate type URL is activated, do not insert it again.
        if($count && $count > 0 && $duplicateTypeUrl) {
            do_action('kdn/category/url/duplicate_url', $postId, $url, $thumbnailUrl, $categoryId);
            return false;
        }
     ...
     * @return false|int
     */
    public function updateUrlSavedStatus($urlId, $saved, $savedPostId = null, $updateCount = 0, $isLocked = false, $lastSavedPostId = null, $lastUrl = null, $cacheProcess = 0, $cacheContent = null) {
        global $wpdb;
        ...
        return $wpdb->update(
            ...
            [
                ...
                'is_locked'             =>  $isLocked,
                'last_saved_post_id'    =>  $lastSavedPostId,
                'last_url'              =>  $lastUrl,
    ...
    public function createDbTables() {
         ...
         * @since 1.6.3
         */
        do_action('kdn/database/after_create', $currentDbVersion);
    ...
    /**
     * Update a URL's stop_crawling_all
     *
     * @param   int     $urlId              ID of the URL
     * @param   string  $stopCrawlingAll    The stop crawling all string
     *
     * @return  false|string
     */
    public function updateUrlStopCrawlingAll($urlId, $stopCrawlingAll = null) {
        global $wpdb;

        return $wpdb->update(
            $this->getDbTableUrlsName(),
            [
                'stop_crawling_all'     =>  $stopCrawlingAll
            ],
            [
                'id'                    =>  $urlId
            ]
    ...
    /**
     * Update a URL's is stop crawling
     *
     * @param int      $urlId       ID of the URL
     * @return false|int
     */
    public function updateUrlStopCrawling($urlId, $lastSavedPostId = null, $savedAt = false, $lastUrl = null) {
        global $wpdb;
        ...
        return $wpdb->update(
            ...
            [
                ...
                'saved_post_id'         =>  null,
                'is_locked'             =>  false,
                'saved_at'              =>  $savedAt ? current_time('mysql') : null,
                'deleted_at'            =>  current_time('mysql'),
                'last_saved_post_id'    =>  $lastSavedPostId,
                'last_url'              =>  $lastUrl,
                'stop_crawling_all'     =>  null
            ],
            [
                'id'        =>  $urlId
            ]
    ...
    /**
     * Update a URL's is bypass delete
     *
     * @param int      $savedPostId       ID of the saved post
     * @return false|int
     */
    public function setUrlBypassDelete($savedPostId) {
        global $wpdb;
