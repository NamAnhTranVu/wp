...
class TestData {
    ...
    /**
     * @var null Find-and-replaces original that will be applied to raw response content for every test that sends a request to
     * remote server. General post and category tests are excluded. They use the settings retrieved from site settings
     * directly. For the structure of this array, see {@link FindAndReplaceTrait::findAndReplace}.
     */
    private $rawHtmlFindReplacesOriginal = null;
    ...
    /**
     * @var array HTML manipulation options original in the test data. E.g. ["_post_exchange_element_attributes" => [...]]
     */
    private $manipulationOptionsOriginal = [];
     ...
     */
    public function getRawHtmlFindReplaces($original = false) {
        return $original ? $this->rawHtmlFindReplacesOriginal : $this->rawHtmlFindReplaces;
    }

    /**
     * @return array See {@link manipulationOptions}
     */
    public function getManipulationOptions($original = false) {
        return $original ? $this->manipulationOptionsOriginal : $this->manipulationOptions;
    }
    ...
    private function prepare() {
        ...
        $this->prepareHtmlManipulationOptionsOriginal();
        $this->prepareUseUtf8();
     ...
     * @return void
     */
    private function prepareHtmlManipulationOptionsOriginal() {
        $manipulationOptions = Utils::array_get($this->data, "manipulation_options_original");
        if (!$manipulationOptions) return;
        ...
        foreach($manipulationOptions as $optionName => $serializedValues) {
            ...
            // Store the options in the instance variable
            $this->manipulationOptionsOriginal[$optionName] = $values;

            // If this option name is for "raw HTML find and replaces", store the value in the related instance variable
            if (!$this->rawHtmlFindReplacesOriginal && ends_with($optionName, 'find_replace_raw_html')) {
                $this->rawHtmlFindReplacesOriginal = $values;
            }
        }
