...
class MediaFile {
    ...
    public function getRandomUniqueHash() {
        return sha1($this->getBaseName() . uniqid('kdn'));
    }
    ...
    public function rename($newName) {
        ...
        /**
        * If do not have a SourceUrl, that means this is a default thumbnail id.
        * Make this is true.
        *
        * @return   true
        * @since    2.1.8
        */
        if ($newName === $this->getName() || !$this->getSourceUrl()) return true;
        ...
        foreach($copyFilePaths as $copyFilePath) {
            ...
            if (@$this->getFileSystem()->move($copyFilePath, $newCopyFilePath)) {
                ...
                Informer::addError(sprintf(_kdn('File %1$s could not be moved to %2$s'), $copyFilePath, $newCopyFilePath))
                    ->addAsLog();
            }
        }
    ...
    public function move($newPath) {
        ...
        if ($result) {
            ...
            // Otherwise, inform the user.
            Informer::addError(sprintf(_kdn('File %1$s could not be moved to %2$s'), $this->getLocalPath(), $newPath))
                ->addAsLog();
        }
    ...
    public function delete() {
        ...
        if (!$result) {
            ...
            Informer::addError(sprintf(_kdn('File %1$s could not be deleted.'), $this->getLocalPath()))
                ->addAsLog();
        }
    ...
    public function deleteCopyFiles() {
        ...
        foreach($this->copyFilePaths as $copyPath) {
            if (!$this->getFileSystem()->delete($copyPath)) {
                ...
                // Inform the user if the file could not be deleted
                Informer::addError(sprintf(_kdn('File %1$s could not be deleted.'), $copyPath))
                    ->addAsLog();
            } else {
    ...
    public function getShortCodeMap() {
        return [
            ShortCodeName::KDN_ITEM                         => function() { return $this->getSourceUrl(); },
    ...
    private function makeDirectory($directoryPath) {
        ...
        if (!$this->getFileSystem()->isDirectory($directoryPath)) {
            ...
            if (!$result) {
                Informer::addError(
                    sprintf(_kdn('%1$s directory could not be created'), $directoryPath)
                )->addAsLog();
