...
class MediaService {
    ...
    public function insertMedia($postId, MediaFile $mediaFile) {
        ...
        $filePath = str_replace(DIRECTORY_SEPARATOR, '/', $mediaFile->getLocalPath());
     ...
     */
    public function saveMedia($fileUrl, $userAgentString = null, $timeoutSeconds = 10, $proxyList = []) {

        // Something went wrong, we need to set default value for $timeoutSeconds manually
        ...
        // Prepare the proxy to download media file
        if (!empty($proxyList)) {
            ...
            foreach ($proxyList as $key => $proxy) {
                ...
                add_action('http_api_curl', function($handle) use ($proxy) {
                ...
                if ($tempFile && !is_wp_error($tempFile)) break;

            }

        // Otherwise, Use default proxy and download file
        } else {
            // Download file to temp dir
            $tempFile = download_url($fileUrl, $timeoutSeconds);
        }
...
//                $type       = $results['type']; // MIME type of the file
                ...
                if(KDNAutoLeech::isDoingTest()) $this->addTestFilePath($results['file']);
                ...
                Informer::add(Information::fromInformationMessage(
                    ...
                    sprintf(_kdn('%1$s, File URL: %2$s'), $results['error'], $fileUrl),
            ...
            Informer::add(Information::fromInformationMessage(
                InformationMessage::FILE_COULD_NOT_BE_SAVED_ERROR,
                sprintf(_kdn('%1$s, File URL: %2$s'), implode($tempFile->get_error_messages(), ' | '), $fileUrl),
                InformationType::INFO
            )->addAsLog());
        }
    ...
    public function removeTestFilePath($path) {
         ...
         *  . The path is not a valid path.
         */
        if (!KDNAutoLeech::isDoingTest() || !$path) return;
