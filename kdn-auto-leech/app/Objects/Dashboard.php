...
namespace KDNAutoLeech\Objects;
...
class Dashboard {
     ...
     *      frequency. Such as ["kdn_event_collect_urls" => "Every 10 minutes", ...]
     */
    ...
    /**
     * @var WP_Post[] An array of WP_Post objects with each having 'kdn' key storing the corresponding URL for that
    ...
    private function initLastSaves() {
        ...
        $this->lastCrawledPosts     = $this->getLastPosts('saved_at', get_option('_kdn_dashboard_count_last_crawled_posts', 10));
        $this->lastRecrawledPosts   = $this->getLastPosts('recrawled_at', get_option('_kdn_dashboard_count_last_recrawled_posts', 10));

        // Prepare last added URLs
        $this->lastUrlsInQueue = $this->getLastAddedUrls(get_option('_kdn_dashboard_count_last_urls', 10));

        // Prepare last deleted URLs
        $this->lastUrlsMarkedAsDeleted = $this->getLastDeletedUrls(get_option('_kdn_dashboard_count_last_deleted_urls', 10));
    }
     ...
     *
     * @param string $eventKey Key of a CRON event defined by the plugin. E.g. "kdn_event_collect_urls"
     ...
     *                          will be used.
     * @return \WP_Post[] An array of {@link WP_Post} instances with each having a 'kdn'
     *                          key which stores the tuple instances with each having a 'kdn' key which stores the
    ...
    /**
     * Adds site data to each URL tuple, under 'kdn' key.
     *
     * @param array $urlTuples An array of URL tuple objects.
     * @return \WP_Post[] Posts for URL tuples, with each post having 'kdn' key that stores the URL tuple.
     */
    private function getPostsFromUrlTuples($urlTuples = []) {
        ...
        foreach($posts as $post) {
            ...
            // Add 'kdn' to the post with 'site' data.
            $post->kdn = $this->addSiteDataToUrlTuples($preparedResults[$post->ID])[0];
        }
