...
class Information {
    ...
    public function __toString() {
        $msg = "KDN Auto Leech ({$this->type}): {$this->message}";

        if ($this->details)     $msg .= " (" . _kdn("Details") . ": {$this->details})";
        if ($this->file)        $msg .= " (" . _kdn("File") . ": {$this->file})";
        if ($this->exception)   $msg .= "\n(" . _kdn("Exception") . ": {$this->exception->getTraceAsString()})";
