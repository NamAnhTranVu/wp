...
class UrlSaver extends AbstractSaver {
    ...
    public $optionLastCheckedSiteId    = '_kdn_last_checked_site_id';
    ...
    public function executeUrlSave($siteIdToCheck) {
        ...
        if(empty($categories)) {
            ...
            error_log("KDN Auto Leech - No category is found for site ID: " . $siteIdToCheck);
            return;
        }
...
//        if(static::$DEBUG) var_dump($categories);
        ...
        if(!$lastCheckedCategoryUrl) {
            ...
            if(!isset($categories[0]["url"]) || !$categories[0]["url"]) {
                $this->resetLastChecked($siteIdToCheck);
                error_log("KDN Auto Leech - Category URL is not valid for site ID: " . $siteIdToCheck);
                return;
            }

            if(!isset($categories[0]["cat_id"]) || !$categories[0]["cat_id"]) {
                $this->resetLastChecked($siteIdToCheck);
                error_log("KDN Auto Leech - Category ID is not valid for site ID: " . $siteIdToCheck);
                return;
            }
...
//        if(static::$DEBUG) var_dump($data);
        ...
        if($data === null) {
            $this->updateLastChecked($siteIdToCheck, $categoryUrlToCheck, false);
            error_log("KDN Auto Leech - Cannot get the data from URL: " . $categoryCurrentPageUrl . " of site: " . $siteIdToCheck);
            return;
        }
...
//                if(static::$DEBUG) var_dump($item);
            if($item["data"]) {
                 ...
                 * @since 1.6.3
                 */
                do_action('kdn/category/url/before_insert_url', $this, $siteIdToCheck, $postUrl, $item);
                ...
                if($urlId = Factory::databaseService()->addUrl($siteIdToCheck, $postUrl, $thumbnailUrl, $targetCategoryId)) {
                     ...
                     * @since 1.6.3
                     */
                    do_action('kdn/category/url/after_insert_url', $this, $siteIdToCheck, $postUrl, $item, $urlId);
                }
            }
        }
        ...
        if($nextPageUrl) {
            if($this->getSetting('_do_not_use_general_settings')) {
                $pageCrawlLimit = $this->getSetting('_kdn_max_page_count_per_category');
                if(!$pageCrawlLimit) $pageCrawlLimit = 0;
            } else {
                $pageCrawlLimit = get_option('_kdn_max_page_count_per_category', 0);
            }
    ...
    private function handleNoNewUrlInsertedCount($siteId, $settings, $nextPageUrl, $insertedCount) {
        if($insertedCount == 0 && $nextPageUrl) {
            ...
            if($this->getSetting('_do_not_use_general_settings')) {
                $trialLimit = $this->getSetting('_kdn_no_new_url_page_trial_limit');
            } else {
                $trialLimit = get_option('_kdn_no_new_url_page_trial_limit');
            }
