...
@if($isGeneralPage)
    <h2 class="nav-tab-wrapper">
        <a href="#" data-tab="#tab-gs-scheduling" class="
            ...
            {{ isset($settings['_kdn_is_scheduling_active']) && !empty($settings['_kdn_is_scheduling_active']) && $settings['_kdn_is_scheduling_active'][0] ? 'nav-tab-highlight-on' : 'nav-tab-highlight-off' }}
        ">
            {{ _kdn('Scheduling') }}
        </a>
        <a href="#" data-tab="#tab-gs-post" class="nav-tab">{{ _kdn('Post') }}</a>
        <a href="#" data-tab="#tab-gs-translation" class="nav-tab">{{ _kdn('Translation') }}</a>
        <a href="#" data-tab="#tab-gs-seo" class="nav-tab">{{ _kdn('SEO') }}</a>
        <a href="#" data-tab="#tab-gs-notifications" class="nav-tab">{{ _kdn('Notifications') }}</a>
        <a href="#" data-tab="#tab-gs-advanced" class="nav-tab">{{ _kdn('Advanced') }}</a>
         ...
         * @since 1.6.3
         */
        do_action('kdn/view/general-settings/add_tab_title', $settings, $isGeneralPage, $isOption);
...
@else
    <div class="section-header-button-container">
        <button class="button" id="btn-load-general-settings">{{ _kdn("Load General Settings") }}</button>
        <button class="button" id="btn-clear-general-settings">{{ _kdn("Clear General Settings") }}</button>
    </div>
@endif
 ...
 * @since 1.6.3
 */
do_action('kdn/view/general-settings/add_tab_content', $settings, $isGeneralPage, $isOption);

?>

{{-- ADVANCED --}}
<div id="tab-gs-advanced" class="tab{{ $isGeneralPage ? '' : '-inside' }} {{ $isGeneralPage ? 'hidden' : '' }}">
    @include('general-settings.tab-advanced', ['isGeneralPage' => $isGeneralPage])
</div>