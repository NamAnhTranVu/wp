    ...
    <div class="raw-html-find-replace-results">
        @foreach($result as $key => $value)
            <div class="result-item">
                <div><span class="result-item-title">{!! $key !!} ({{ _kdn('Length') }}: {{ mb_strlen($value) }}):</span></div>
                @if($value)
                    <textarea class="large">{{ $value }}</textarea>
                @else
                    <span>{{ _kdn('No HTML. If you expect HTML, make sure the HTML is valid or there is network connection.') }}</span>
                @endif
            </div>
        @endforeach
    </div>
